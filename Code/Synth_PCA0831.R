...
# Read data
usa.shen <- read.csv("Data/szcpr-0710.csv", header = T, fileEncoding = "GBK")
...
# write.csv(synthw[order(synthw$w.weights, decreasing = T),],"Result/weight0711.csv")
...
ggplot(data = placebo.usashen,mapping = aes(x = date, y = per.confirm, group = area)) +
  geom_line(aes(color = group,alpha = group),size = 0.8) +
...
trend.usa
# ggsave("Result/shenzhen_0712.pdf", device = cairo_pdf, width = 8, height = 6)
